...
class QuestionModel(models.Model):
    ...
    def clean_answers(self):
        ...
        if len(self.answers) != len(ANSWERS_STRUCTURE):
            raise ValidationError(f"El campo 'answers' debe tener exactamente {len(ANSWERS_STRUCTURE)} elementos.")
        
        for expected_answer, received_answer in zip(ANSWERS_STRUCTURE, self.answers):
            if not isinstance(received_answer, dict):
                raise ValidationError("Cada elemento en 'answers' debe ser un diccionario.")
            
            for key, expected_type in expected_answer.items():
                if key not in received_answer:
                    raise ValidationError(f"Falta la clave '{key}' en uno de los elementos de 'answers'.")
                
                if not isinstance(received_answer[key], expected_type):
                    raise ValidationError(f"El valor de '{key}' en 'answers' debe ser de tipo {expected_type}.")